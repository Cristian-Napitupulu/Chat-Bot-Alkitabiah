...
data_folder = "/intents"
images_folder = "/gambar"
data_path = script_path + data_folder
...
df = pd.read_excel(data_path + "/olah_intents.xlsx")
...
bar1 = ax.bar(
    index, number_of_unique_patterns, bar_width, label="Number of Unique Patterns"
)
bar2 = ax.bar(
...
plt.subplots_adjust(
    top=0.9, bottom=0.3, left=0.06, right=0.965
)  # Adjust bottom padding as needed
ax.legend()
plt.grid(axis="y")

plt.savefig(images_path + "/" + plot_name + ".svg", format="svg", transparent=True)
plt.savefig(images_path + "/" + plot_name + ".png")
